...
LOGS = logging.getLogger("اعداد ميرو")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@SourceMiro"))
        await sbb_b(JoinChannelRequest("@qqqqJp"))
        await sbb_b(JoinChannelRequest("@Help_SourceMiro"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/84d3a8b86e28830c38fed.jpg",
                caption="**تم تشغيل سورس ميرو بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/help_SourceMiro"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كروب بوت ميرو", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
